...
class ToolExecutor:
    ...
    def evaluate(
        ...
        *,
        ...
        if not self.actions_allowed and spec.mutation_class.requires_abac():
            return ToolResult(
                status=ToolResultStatus.REFUSED,
                call_digest=call_digest,
                reason_code="actions_disabled",
                reason="side-effecting actions are disabled by operator toggle",
            )
...
def evaluate_tool_call(
    ...
    *,
    ...
    return ToolExecutor(registry, invariant=invariant, actions_allowed=actions_allowed).evaluate(
