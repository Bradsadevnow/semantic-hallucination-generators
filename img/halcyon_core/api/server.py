...
from halcyon_core.governance.claims import claim_from_verified_receipt
    ...
    *,
    ...
    def health() -> dict[str, Any]:
        ...
        return {
            ...
            "actions_allowed": config.actions_allowed,
    ...
    @app.get("/governance/receipts")
    def list_governance_receipts() -> dict[str, Any]:
        ledger = SQLiteEventLedger(config.events_database_path())
        try:
            return {"receipts": list(_iter_receipt_summaries(ledger))}
        finally:
            ledger.close()

    @app.get("/governance/receipts/{receipt_id}")
    def get_governance_receipt(receipt_id: str) -> dict[str, Any]:
        ledger = SQLiteEventLedger(config.events_database_path())
        try:
            record = _find_receipt_record(ledger, receipt_id)
        finally:
            ledger.close()
        if record is None:
    ...
    @app.get("/governance/receipts/{receipt_id}/verify")
    def verify_governance_receipt(receipt_id: str) -> dict[str, Any]:
        ledger = SQLiteEventLedger(config.events_database_path())
        try:
            record = _find_receipt_record(ledger, receipt_id)
        finally:
        ...
        if record is None:
            raise HTTPException(status_code=404, detail="receipt not found")
        return {"receipt_id": receipt_id, "verification": record["verification"]}
...
def _iter_receipt_summaries(ledger: SQLiteEventLedger):
    for event in ledger.iter_events_by_name("governance.receipt.emitted"):
        ...
        yield {
            ...
            "call_digest": payload.get("call_digest"),
...
def _find_receipt_record(ledger: SQLiteEventLedger, receipt_id: str) -> dict[str, Any] | None:
    for event in ledger.iter_events_by_name("governance.receipt.emitted"):
        payload = event.payload
        receipt = payload.get("receipt", {})
        identity = receipt.get("identity", {})
        if identity.get("receipt_id") != receipt_id:
            continue
        verification = verify_receipt(receipt, expected_digest=payload.get("receipt_digest"))
        return {
            ...
            "derived_claim": claim_from_verified_receipt(
                receipt,
                expected_digest=payload.get("receipt_digest"),
            ).to_dict(),
        }
    return None
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--actions-allowed", action="store_true", help="Enable side-effecting governed actions.")
...
def startup_summary(config: RuntimeConfig, *, host: str, port: int, allow_external: bool = False) -> str:
    ...
    lines = [
        ...
        f"ActionsAllowed: {'ON' if config.actions_allowed else 'OFF'}",
        f"Exposure: {exposure}",
    ]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    config = RuntimeConfig(
        ...
        actions_allowed=args.actions_allowed,
    )
...
app = create_app(RuntimeConfig(
    ...
    actions_allowed=_os.environ.get("HALCYON_ACTIONS_ALLOWED", "").lower() in {"1", "true", "yes", "on"},
))
