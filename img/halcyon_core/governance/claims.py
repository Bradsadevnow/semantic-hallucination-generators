...
from halcyon_core.governance.receipts import ReceiptVerification, verify_receipt
...
class ClaimEvidence:
    ...
    truth_source: str = "runtime_evidence"
    ...
    def classify(
        ...
        *,
        ...
        return FinalClaim(
            ...
            proof_artifact_id=evidence.proof_artifact_id,
...
def claim_from_verified_receipt(
    receipt: dict[str, object],
    *,
    expected_digest: str | None = None,
) -> FinalClaim:
    verification = verify_receipt(receipt, expected_digest=expected_digest)
    identity = receipt.get("identity", {}) if isinstance(receipt, dict) else {}
    decision = receipt.get("decision", {}) if isinstance(receipt, dict) else {}
    governance_inputs = receipt.get("governance_inputs", {}) if isinstance(receipt, dict) else {}

    if not verification.valid:
        return ClaimClassifier().classify(
            ClaimEvidence(
                execution_path_id=str(identity.get("call_digest", "receipt_path")),
                conditions={
                    "C1": "breach",
                    "C2": "breach",
                    "C3": "breach",
                ...
                breach_notices=verification.failures,
                scope_limitations=("receipt_verification_failed",),
                epistemic_taint=("receipt_invalid",),
                segment_documented=True,
                truth_source="verified_receipt",
                proof_artifact_id=identity.get("receipt_id"),
                proof_artifact_digest=verification.receipt_digest,
            )
        )
    ...
    if governance_inputs.get("policy_snapshot", {}).get("actions_allowed") is False:
        conditions["C6"] = "fail"
    ...
    claim = ClaimClassifier().classify(
        ClaimEvidence(
            ...
            conditions=conditions,
            scope_limitations=(
                f"receipt_outcome={decision.get('outcome', 'unknown')}",
                "claim_derived_from_verified_receipt",
            ),
            segment_documented=True,
            truth_source="verified_receipt",
            proof_artifact_id=identity.get("receipt_id"),
