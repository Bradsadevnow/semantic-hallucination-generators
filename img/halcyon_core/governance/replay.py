...
class ToolChain:
    ...
    receipt_event: RuntimeEvent | None = None
    ...
    def has_receipt(self) -> bool:
        return self.receipt_event is not None
    ...
    def governance_claim(self) -> str:
        ...
        if self.execution_outcome == "refused":
            if not self.has_approval:
                return "incomplete"
            if self.receipt_violations:
        ...
        if self.ordering_violations or self.digest_violations or self.parent_violations or self.receipt_violations:
            return "governance_breach"
    ...
    def all_violations(self) -> list[str]:
        return (
            ...
            + self.parent_violations
            + self.receipt_violations
        )
...
def reduce_replay_evidence(ledger: SQLiteEventLedger) -> ReplayEvidence:
    ...
    for event in ledger.iter_events():
        ...
        elif name == "governance.receipt.emitted":
            call_digest = p.get("call_digest")
            if call_digest:
                chain = tool_chains.setdefault(call_digest, ToolChain(call_digest=call_digest))
                chain.receipt_event = event
...
def _validate_tool_chain(chain: ToolChain) -> None:
    ...
    if chain.receipt_event is not None:
        receipt_payload = chain.receipt_event.payload.get("receipt")
        receipt_digest = chain.receipt_event.payload.get("receipt_digest")
        if receipt_payload is None:
            chain.receipt_violations.append("missing receipt payload")
            return
        verification = verify_receipt(receipt_payload, expected_digest=receipt_digest)
        chain.receipt_violations.extend(verification.failures)

        if receipt_payload.get("identity", {}).get("call_digest") != chain.call_digest:
            chain.receipt_violations.append("receipt call_digest does not match chain")

        receipt_outcome = receipt_payload.get("decision", {}).get("outcome")
        if receipt_outcome != chain.execution_outcome:
            chain.receipt_violations.append(
                f"receipt outcome {receipt_outcome!r} does not match execution outcome {chain.execution_outcome!r}"
            )

        if chain.approval_event is not None:
            receipt_action_digest = receipt_payload.get("identity", {}).get("action_digest")
            approval_action_digest = chain.approval_event.payload.get("action_digest")
            if receipt_action_digest != approval_action_digest:
                chain.receipt_violations.append("receipt action_digest does not match approval event")
