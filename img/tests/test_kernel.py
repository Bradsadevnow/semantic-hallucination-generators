...
def test_kernel_durable_memory_confirmation_does_not_create_memory_events(tmp_path):
    captured = {}
    kernel = build_kernel_for_path(
        tmp_path / "halcyon.sqlite",
            ...
            json.dumps(
                {
                    ...
                    "memory_proposals": [
                        {"content": "Brad prefers MVL before expansion.", "reason": "test"}
                    ],
                    "tool_proposals": [],
                }
            ),
    ...
    turn = kernel.handle_message("remember Brad prefers MVL before expansion")
    event_names = [event.name for event in kernel.events.events]

    assert "durable memory" in turn.reply.lower()
    assert turn.model_event is None
    assert turn.interpretation.status.value == "needs_confirmation"
    assert turn.memory_proposals == ()
    assert captured == {}
    assert "interpretation.confirmation_requested" in event_names
    assert "memory.proposal.created" not in event_names
    assert "memory.durable.created" not in event_names
    assert kernel.proposals.pending() == []
    assert kernel.memory.recent() == []
    kernel.close()
