from halcyon_core.governance.claims import (
    ClaimClassifier,
    ClaimEvidence,
    claim_from_verified_receipt,
    classify_claim,
)
from halcyon_core.governance.receipts import build_executed_receipt, digest_receipt
...
def test_claim_can_be_derived_from_verified_receipt():
    receipt = build_executed_receipt(
        emitted_at="2026-04-23T12:00:00+00:00",
        request_id="event:1",
        call_digest="sha384:call",
        action_digest="sha384:action",
        lineage={"proposal_event_index": 1, "binding_event_index": 2, "approval_event_index": 3, "decision_event_index": 4},
        policy_snapshot={"governance_mode": "deny_by_default", "actions_allowed": True},
        authority_snapshot={"approved_by": "brad", "approval_basis": "test", "approval_digest": "sha384:approval", "action_digest": "sha384:action", "binding_status": "bound", "override_applied": False, "binding_reason_codes": ["tool_name_in_interpreted_intent"]},
        runtime_posture_snapshot={"actions_allowed": True, "health_state": "not_evaluated", "containment_state": "not_evaluated", "unresolved_anomaly_count": 0, "last_governing_failure_class": "unknown"},
        boundary_evidence={"invariant_digest": "sha384:inv"},
        boundary_attestations=[
            {"boundary_name": "proposal_boundary", "decision": "approved", "reason_codes": ["proposal_recorded"], "evidence": ["proposal"], "snapshot_digests": {"proposal_event_digest": "sha384:p"}, "event_index": 1},
            {"boundary_name": "authority_boundary", "decision": "approved", "reason_codes": ["explicit_tool_approval"], "evidence": ["approval"], "snapshot_digests": {"approval_event_digest": "sha384:a"}, "event_index": 3},
            {"boundary_name": "execution_boundary", "decision": "approved", "reason_codes": ["executed"], "evidence": ["execution"], "snapshot_digests": {"execution_event_digest": "sha384:e"}, "event_index": 4},
        ],
        output={"echoed": "ping"},
        reason_code="executed",
        governing_predicate_class="approval_bound_execution",
    )

    claim = claim_from_verified_receipt(receipt.to_dict(), expected_digest=digest_receipt(receipt))

    assert claim.truth_source == "verified_receipt"
    assert claim.proof_artifact_id == receipt.identity.receipt_id
    assert claim.proof_artifact_digest == digest_receipt(receipt)
    assert "claim_derived_from_verified_receipt" in claim.scope_limitations
