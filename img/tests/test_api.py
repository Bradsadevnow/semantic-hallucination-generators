...
def test_api_health_returns_ok(tmp_path):
    ...
    assert data["actions_allowed"] is False
...
def test_server_startup_summary_declares_boundary(tmp_path):
    ...
    assert "ActionsAllowed: OFF" in local
...
def test_server_parser_defaults_to_localhost():
    ...
    assert args.actions_allowed is False
...
def test_api_chat_reuses_kernel_health_state_across_turns(tmp_path):
    ...
    assert first["tool_results"][0]["reason_code"] == "actions_disabled"
    assert second["tool_results"][0]["reason_code"] == "actions_disabled"
...
def test_api_memory_proposal_list_approve_reject(tmp_path):
    ...
    store = SQLiteMemoryStore(config.memory_database_path())
    try:
        stored_memory = store.get(durable_event.payload["memory_id"])
    finally:
        store.close()
    ...
    assert stored_memory.content == "api proposal"
    assert stored_memory.metadata["proposal_id"] == first.proposal_id
...
def test_api_lists_and_fetches_verified_governance_receipts(tmp_path):
    config = RuntimeConfig(database_path=str(tmp_path / "halcyon.sqlite"), actions_allowed=True)
    app = create_app(config, model_client=fake_client(tool_proposal_reply()))
    client = TestClient(app)

    turn = client.post("/chat", json={"message": "echo hi"}).json()
    ...
    listed = client.get("/governance/receipts")
    receipts = listed.json()["receipts"]

    assert listed.status_code == 200
    assert len(receipts) == 1
    receipt_id = receipts[0]["receipt_id"]
    assert receipts[0]["call_digest"] == call_digest
    assert receipts[0]["outcome"] == "executed"

    fetched = client.get(f"/governance/receipts/{receipt_id}")
    fetched_data = fetched.json()

    assert fetched.status_code == 200
    assert fetched_data["receipt_id"] == receipt_id
    assert fetched_data["receipt"]["identity"]["call_digest"] == call_digest
    assert len(fetched_data["receipt"]["boundary_attestations"]) == 4
    assert fetched_data["receipt"]["boundary_attestations"][-1]["boundary_name"] == "execution_boundary"
    assert fetched_data["verification"]["valid"] is True
    assert fetched_data["verification"]["failures"] == []
    assert fetched_data["derived_claim"]["truth_source"] == "verified_receipt"
    assert fetched_data["derived_claim"]["proof_artifact_id"] == receipt_id
    assert fetched_data["derived_claim"]["proof_artifact_digest"] == fetched_data["receipt_digest"]

    verified = client.get(f"/governance/receipts/{receipt_id}/verify")
    verified_data = verified.json()

    assert verified.status_code == 200
    assert verified_data["receipt_id"] == receipt_id
    assert verified_data["verification"]["valid"] is True


def test_api_exposes_refused_receipts_as_first_class_artifacts(tmp_path):
    config = RuntimeConfig(database_path=str(tmp_path / "halcyon.sqlite"))
    app = create_app(
        config,
    ...
    turn = client.post("/chat", json={"message": "write_file hi"}).json()
    call_digest = turn["tool_results"][0]["call_digest"]
    approval = client.post(
        f"/tools/proposals/{call_digest}/approve",
        json={"approved_by": "brad", "approval_basis": "user_direct_consent"},
    ...
    receipts = client.get("/governance/receipts").json()["receipts"]
    assert len(receipts) == 1
    assert receipts[0]["outcome"] == "refused"

    receipt_id = receipts[0]["receipt_id"]
    fetched = client.get(f"/governance/receipts/{receipt_id}").json()

    assert fetched["receipt"]["decision"]["outcome"] == "refused"
    assert fetched["receipt"]["decision"]["refusal_artifact"]["reason_code"] == "actions_disabled"
    assert fetched["receipt"]["governance_inputs"]["policy_snapshot"]["actions_allowed"] is False
    assert fetched["receipt"]["boundary_attestations"][-1]["decision"] == "denied"
    assert fetched["verification"]["valid"] is True
    assert fetched["derived_claim"]["truth_source"] == "verified_receipt"


def test_api_actions_allowed_toggle_fail_closes_approved_execution(tmp_path):
    config = RuntimeConfig(database_path=str(tmp_path / "halcyon.sqlite"))
    app = create_app(config, model_client=fake_client(tool_proposal_reply()))
    client = TestClient(app)
    ...
    assert detail["reason_code"] == "actions_disabled"
    assert client.get("/health").json()["actions_allowed"] is False


def test_api_receipt_lookup_404s_for_missing_receipt(tmp_path):
    app = create_app(
        RuntimeConfig(database_path=str(tmp_path / "halcyon.sqlite")),
        model_client=fake_client(json.dumps({"reply": "ok", "memory_proposals": [], "tool_proposals": []})),
    )
    ...
    missing = client.get("/governance/receipts/receipt:missing")
    missing_verify = client.get("/governance/receipts/receipt:missing/verify")
